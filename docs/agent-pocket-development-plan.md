# Kaka / Pocket Agent Development Plan

Updated: 2026-06-04

This is the execution-level development plan for Kaka / Pocket Agent. It reconciles the current codebase, the current product direction, and the tools that should be used to implement the next milestones.
...
The broader product direction is Pocket Agent: Kaka becomes the voice-first phone front end for local agents. That direction should build on the proven image loop rather than replace it. See `docs/pocket-agents-direction.md`.

## Current Truth

The codebase is already a working Pocket Agent MVP foundation, but the current shippable product loop is not complete.
...
| Runtime Kit | Development scaffold for explicit-start Pocket Agent Mobile Bridge packaging exists; Hermes/OpenClaw plugin packaging is planned, not production-ready | `runtime-kit`, `docs/kaka-runtime-kit-plan.md` |
...
| Pocket Agent direction | Product direction and HTML UI prototype documented; Share to Pocket Agent, voice, Context Snapshot, screenshot Q&A, and Recall are planned, not implemented | `docs/pocket-agents-direction.md`, `docs/ui/kaka-pocket-agents-prototype.html`, `docs/agent-pocket-privacy.md`, `docs/mobile-bridge-api.md` |
...
- Treat Pocket Agent as the post-image-loop product expansion: share-sheet intake, voice-first follow-up, permissioned context, screenshot Q&A, and explicit Recall.
...
- Do not make normal users paste long terminal commands to connect. The consumer path should be a Hermes/OpenClaw plugin or skill UI with a visible **Pocket Agent Mobile Bridge** enable/start control. Install must not auto-start a LAN bridge.
...
| Pocket Agent docs | `docs/pocket-agents-direction.md`, `docs/agent-pocket-privacy.md` | Post-image-loop product direction, privacy boundaries, future universal intake | Must remain clearly marked as planned until endpoints and iOS surfaces exist |
...
Purpose: prove Pocket Agent can make a photo visibly better without any cloud image generation.
...
- Runtime plugin/skill install does not start a listener; users explicitly enable **Pocket Agent Mobile Bridge**.
...
### Phase 7: Pocket Agent Expansion
...
1. **Share to Pocket Agent Inbox**: receive text, URLs, screenshots, images, PDFs, and small files from the system share sheet.
...
- Pocket Agent can show what context was sent with a task.
...
| G. Pocket Agent | Share to Pocket Agent, voice, context snapshot, screenshot Q&A, Recall | `docs/pocket-agents-direction.md`, future iOS extension/runtime intake files | Share Extension/App Intents/Speech docs, Swift tests, bridge contract tests |
...
| P1 | Pocket Agent scope expands into hidden surveillance or unsafe automation | Keep all inputs user-initiated; require confirmation for memory, posting, deletion, payment, messaging, and tool execution. |
...
| P3 | Naming drift between Kaka, Pocket Agent, Hermes, runtime | Brand externally as Kaka; keep Pocket Agent as module/app identifier until renamed intentionally. |
...
Continue with Phase 2/3 evidence closure, then return to Phase 5 visual polish. Pocket Agent planning can continue in docs, but implementation should start after the image loop has enough evidence to stay stable.
...
8. After Gate F evidence is healthy, write a focused implementation plan for Share to Pocket Agent Inbox and voice follow-up as the first Pocket Agent slice.
...
- Whether Pocket Agent keeps Pocket Agent as the internal module name or renames modules later.
...
- Whether Pocket Agent becomes a visible product/category name or remains an internal roadmap label under Kaka.
...
- Which Share to Pocket Agent content types ship first: text/URL/image only, or PDF/audio as well.
