# Pocket Agent 开发章程(Delivery Charter)
...
Pocket Agent 已经从单一拍照/修图原型演进成 iPhone 本地智能体入口:连接 Hermes、OpenClaw 或兼容 Mobile Bridge 的本机运行时,由手机负责采集、预览、语音、收件箱、确认和状态展示,由运行时负责模型、工具、记忆、任务和保留策略。
...
本章程把 Pocket Agent 的近期开发重新定义为:

> Pocket Agent 先做本地局域网内的手机感知入口(Local Agent Lens),解决用户装好 Hermes/OpenClaw 后不知道能做什么的问题;云端和公网远程访问暂不作为当前阶段目标。
...
6. **安装路径 host-native**:普通用户未来应安装 Hermes Plugin 或 OpenClaw Skill/sidecar,在宿主 UI 中启用 Pocket Agent Mobile Bridge,再扫码或 Bonjour 配对。Codex skill/plugin 只能是 host-team 开发自动化,不是普通用户安装面。
7. **Dynamic Island 使用公开 ActivityKit**:Pocket Agent 可以做 Live Activities / Dynamic Island compact、minimal、expanded 状态,但不能声称复制或依赖 Apple 私有 Siri UI。
...
- 在用户已连接并要求真机测试时安装/启动当前 Pocket Agent dev app。
...
目标:用户安装 Pocket Agent 和本机 Hermes/OpenClaw 后,知道如何连接、断线后知道如何恢复,同一 Wi-Fi/LAN 下能稳定配对和重新连接。
...
目标:解决“装好智能体不知道干什么”的第一公里。Pocket Agent 打开后直接给用户手机动作,而不是空聊天框。

范围:

1. Agent Scanner:扫码/文字扫描后展示下一步动作,包括 URL 总结、打开、复制、保存到 Inbox、Pocket Agent pairing QR 连接。
...
目标:让 Pocket Agent 能处理短视频任务,并用 Live Activities / Dynamic Island 展示本地智能体状态。
...
- `docs/pocket-agents-direction.md`:Pocket Agent 产品方向背景。
