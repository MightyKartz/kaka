...
多数 AI 手机助手和照片应用会把用户数据和模型凭证搬进云服务。Kaka 坚持更窄的本地优先边界：
...
产品目标是一个可靠的口袋智能体闭环：拍下或分享一样东西，让 Kaka 说明它能做什么，通过点按、文字或语音继续，并由用户决定什么应该被记住。
...
| `ios/KakaShareExtension` | 文本、URL、图片、PDF 采集的 Share Extension target |
...
| `docs` | API 文档、隐私文档、开发计划、Pocket Agents 方向和 UI/UX 原型 |
